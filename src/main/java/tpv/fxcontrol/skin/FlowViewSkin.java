...
import tpv.fxcontrol.behavior.FlowViewBehavior;
 ...
 * @since 9
 */
public class FlowViewSkin<T> extends VirtualContainerBase<FlowView<T>, FlowCell<T>> {
    ...
    private FlowViewBehavior<T> behavior;
     ...
     */
    public FlowViewSkin(final FlowView<T> control) {
        super(control);
    ...
    private int onScrollPageUp(boolean isFocusDriven) {
        ...
        if (isSelected) {
            ...
            if (isLeadIndex) {
                ...
                FlowCell<T> newFirstVisibleCell = flow.getFirstVisibleCellWithinViewport();
