 ...
 * to the API defined on Cell and {@link FlowIndexedCell}, the ListCell is more
...
public class FlowCell<T> extends FlowIndexedCell<T> {
    ...
    private final ListChangeListener<T> itemsListener = c -> {
        ...
        while (c.next()) {
            ...
            final FlowView<T> lv = getListView();
    ...
    private final InvalidationListener itemsPropertyListener = new InvalidationListener() {
        ...
        @Override public void invalidated(Observable observable) {
            ...
            FlowView<T> listView = getListView();
     ...
     */
    private ReadOnlyObjectWrapper<FlowView<T>> listView = new ReadOnlyObjectWrapper<FlowView<T>>(this, "listView") {
        /**
         * A weak reference to the ListView itself, such that whenever the ...
         */
        private WeakReference<FlowView<T>> weakListViewRef = new WeakReference<FlowView<T>>(null);

        @Override protected void invalidated() {
            // Get the current and old list view references
            final FlowView<T> currentListView = get();
            final FlowView<T> oldListView = weakListViewRef.get();
            ...
            if (currentListView != null) {
                ...
                weakListViewRef = new WeakReference<FlowView<T>>(currentListView);
            }
    ...
    private void setListView(FlowView<T> value) { listView.set(value); }
    public final FlowView<T> getListView() { return listView.get(); }
    public final ReadOnlyObjectProperty<FlowView<T>> listViewProperty() { return listView.getReadOnlyProperty(); }
...
//    @Override
    ...
    @Override protected Skin<?> createDefaultSkin() {
        return new FlowCellSkin<T>(this);
    }
    ...
    @Override public void startEdit() {
        if (isEditing()) return;
        final FlowView<T> list = getListView();
     ...
     */
    public final void updateListView(FlowView<T> listView) {
